# ustreader.py
from ust_types import CoreUstInfo, NoteInfo


def get_ust_info(ust_folder: str, encoding: str) -> CoreUstInfo:
    # ===== 1. 初始化要提取的信息（本地化变量）=====
    ust_version: str = ""          # 存储UST版本
    ust_tempo: float = 0.0         # 存储速度
    ust_tracks: int = 0            # 存储轨道数
    note_list: list[NoteInfo] = [] # 存储所有音符信息（列表里放字典）
    current_note: NoteInfo | None = None  # 临时存储当前解析的音符
    ...
    try:
        ...
        for line in ust_lines:
            ...
            # 3. 提取音符分段内的信息
            elif in_note and "=" in line and current_note is not None:
